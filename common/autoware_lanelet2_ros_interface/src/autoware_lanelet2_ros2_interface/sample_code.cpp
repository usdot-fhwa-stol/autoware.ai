...
class autoware_lanelet2_ros2_interface_example : public rclcpp::Node
{
    public: 

    autoware_lanelet2_ros2_interface_example()
        : Node("autoware_lanelet_ros2_interface")
    {
    ...
    void activate(const rclcpp_lifecycle::State &){
        ...
        loadingAutowareOSMFile(map_file_path_);
        usingMGRSProjector();
        usingAutowareTrafficLight(map_file_path_);

        rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn SUCCESS;
    }
    
    private:
    // Private variable to store map file path passed as parameter
...
int main(int argc, char* argv[])
{
    ...
    auto node = std::make_shared<autoware_lanelet2_ros2_interface_example>();
