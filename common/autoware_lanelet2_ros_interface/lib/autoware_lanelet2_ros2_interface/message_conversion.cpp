...
void toBinMsg(const lanelet::LaneletMapPtr& map, autoware_lanelet2_msgs::msg::MapBin* msg)
{
    if (msg == nullptr)
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("lanelet::utils::conversion"),__FUNCTION__ << "msg is null pointer!");
        return;
    }
...
void fromBinMsg(const autoware_lanelet2_msgs::msg::MapBin& msg, lanelet::LaneletMapPtr map)
{
  if (!map)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("lanelet::utils::conversion"),__FUNCTION__ << ": map is null pointer!");
    return;
  }
...
void toGeomMsgPt(const geometry_msgs::msg::Point32& src, geometry_msgs::msg::Point* dst)
{
  if (dst == nullptr)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("lanelet::utils::conversion"), __FUNCTION__ << "pointer is null!");
    return;
  }
  dst->x = src.x;
  dst->y = src.y;
