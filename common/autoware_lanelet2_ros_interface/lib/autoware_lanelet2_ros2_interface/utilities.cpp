                                ...
                                std::map<int, std::vector<int> >* wp_candidate_lanelets, const bool reverse)
{
  if (!lanelet_map)
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("lanelet::utils"), "No lanelet map is set!");
    return;
  }
                           ...
                           std::vector<int>* contacting_lanelet_ids)
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("lanelet::utils"), __FUNCTION__ << " contacting_lanelet_ids is null pointer!");
    return;
  }
...
void matchWaypointAndLanelet(const lanelet::LaneletMapPtr lanelet_map,
                             const lanelet::routing::RoutingGraphPtr routing_graph,
                             const autoware_msgs::msg::LaneArray& lane_array,
                             std::map<int, lanelet::Id>* waypointid2laneletid)
{
  ...
  {
    RCLCPP_ERROR_STREAM(rclcpp::get_logger("lanelet::utils"), __FUNCTION__ << ": waypointid2laneletid null pointer!");
    return;
  }
  ...
  for (auto candidate : wp_candidate_lanelet_ids)
  {
    ...
    {
      RCLCPP_WARN_STREAM(rclcpp::get_logger("lanelet::utils"), "No lanelet was matched for waypoint with gid: " << candidate.first);
      continue;
    }
    if (candidate.second.size() >= 2)
    {
      RCLCPP_WARN(rclcpp::get_logger("lanelet::utils"),"ambiguous waypoint. Randomly choosing from candidates");
    }
